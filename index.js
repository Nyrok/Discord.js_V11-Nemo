...
bot.on("ready", () => {
  console.log(figlet.textSync("<némo>\nBy Nyrok and Plattyz").cyan);
  let statues = ["@Nyrok10 and @0x4d6f6b6d69 on Twitter", "github.com/Nyrok", `"." is the basic prefix.`, "Nemo-Project", "github.com/Plattyz"];
...
bot.on("guildCreate", guild => {
  ...
  const hiy = new discord.RichEmbed()
    ...
    .setFooter(
      "@Nyrok10 and @0x4d6f6b6d69 on Twitter",
...
bot.on("message", message => {
  ...
  const prefix = db.get("config").find({
    id: message.guild.id
  }).get("prefix").value();
  ...
  if (message.content.startsWith(prefix + "name")) {
    ...
    if (!message.content.startsWith(prefix + "name ")) {
      return message.channel.send(":x: Vous devez préciser le nouveau nom du bot!");
    }
    const klou = message.content.replace(prefix + "name ", "");
    message.guild.member(bot.user).setNickname(klou);
  }
  ...
  if (message.content === prefix + "hentai") {
    ...
    if (message.channel.nsfw === true) {
      ...
      message.channel.send(":x: Vous devez aller dans un salon NSFW!");
    }
  }
